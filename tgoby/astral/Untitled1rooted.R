...
tre <- read.newick(file="uces.astral-hybrid.2.tre", node.label="support")

tre@data

ggt <- ggtree(tr=tre, layout="slanted")
...
write_tsv(x=ggt$data, file="ggt-data.2.tsv")
ggt$data <- read_tsv(file="ggt-data.2.tsv")
...
gg <- 
  ggt + 
  geom_hilight(mapping=aes(subset=node%in%c(132, 109, 86), fill=I(COLOR1)), type="gradient", gradient.direction='tr', alpha=0.2, to.bottom=TRUE, align="right", extend=1) +
  ...
  geom_tree(mapping=aes(color=I(COLOR1)), layout="slanted") + 
  geom_tree(mapping=aes(size=I(support), color=I(COLOR1)), continuous="size", layout="slanted") + 
  ...
  geom_cladelab(data=poplabs, mapping=aes(node=node, label=name, colour=I(color)), align=TRUE, lineheight=0.8, barsize=0, barcolour=NA, hjust=0.5, offset=0.14, fontsize=3.5) +
  ...
  scale_size_identity(guide="legend", name="Posterior Probability", breaks=c(0.4, 0.6, 0.8, 1.0), labels=c(0.4, 0.6, 0.8, 1.0)) +
  xlim(c(NA, 2.5)) + 
  theme(
...
ggsave(plot=gg, filename="uces.astral-hybrid.2r.png", width=4, height=8, device=png, dpi=300, units="in")
ggsave(plot=gg, filename="uces.astral-hybrid.2r.pdf", width=4, height=8, device=cairo_pdf)
