...
ggsave(plot=g, filename="WGS_vs_Kinziger_et_al_2015-facet-boots.png", device=png, width=8, height=2, units="in", dpi=300)
ggsave(plot=g, filename="WGS_vs_Kinziger_et_al_2015-facet-boots.pdf", device=cairo_pdf, width=8, height=2, units="in")
...
gg <- 
  ggplot() +
  geom_abline(data=boot_dat, mapping=aes(slope=value, intercept=0), alpha=0.05, col="gray", linewidth=0.5) +
  geom_abline(lty=2, linewidth=0.3) + 
  geom_abline(data=cor.obs, mapping=aes(slope=correlation, intercept=0)) +
  geom_point(data=dat, mapping=aes(x=STR, y=SNP), shape=1, alpha=1, size=2) + 
  theme_base(base_size=10) +
...
ggsave(plot=gg, filename="WGS_vs_Kinziger_et_al_2015-facet-fst~km.png", device=png, width=8, height=3, units="in", dpi=300)
ggsave(plot=gg, filename="WGS_vs_Kinziger_et_al_2015-facet-fst~km.pdf", device=cairo_pdf, width=8, height=3, units="in")
