...
g <- 
  ...
  scale_x_continuous(expand=expansion(mult=c(0.02, 0.02)), breaks=c(0.0011, 0.01, 0.1, 0.95)) +
  ...
  theme(
    ...
    axis.text=element_text(size=10, hjust=0.7), 
...
ggsave(plot=g, filename="facet-boots3.png", device=png, width=3, height=1, units="in", dpi=300)
ggsave(plot=g, filename="facet-boots3.pdf", device=cairo_pdf, width=3, height=1, units="in")
...
gg <- 
  ggplot(filter(boot_aug, callset!="SNPs"), aes(x=KM, y=`FST/(1-FST)`)) +
  geom_line(aes(y=.fitted, group=id), col="lightgray") +
  geom_smooth(data=filter(dat, callset!="SNPs"), mapping=aes(x=KM, y=`FST/(1-FST)`), method="lm", se=FALSE, color="black", linewidth=0.5) +
  geom_point(data=filter(dat, callset!="SNPs"), mapping=aes(x=KM, y=`FST/(1-FST)`), pch=1) + 
  facet_wrap(facets=vars(callset), ncol=2, nrow=1, scales="fixed") +
  theme_base(base_size=10) +
  labs(y=expression(italic(F)[ST]/(1-italic(F)[ST]))) + 
  theme(
...
ggsave(plot=gg, filename="facet-fst~km3.png", device=png, width=3, height=2, units="in", dpi=300)
ggsave(plot=gg, filename="facet-fst~km3.pdf", device=cairo_pdf, width=3, height=2, units="in")
