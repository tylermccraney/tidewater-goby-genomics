...
ggsave(plot=g, filename="facet-boots2.png", device=png, width=6, height=1.5, units="in", dpi=300)
ggsave(plot=g, filename="facet-boots2.pdf", device=cairo_pdf, width=6, height=1.5, units="in")
...
gg <- 
  ...
  geom_smooth(data=filter(dat, callset!="SNPs"), mapping=aes(x=KM, y=`FST/(1-FST)`), method="lm", se=FALSE, color="black", linewidth=0.5, alpha=1) +
  geom_point(data=filter(dat, callset!="SNPs"), mapping=aes(x=KM, y=`FST/(1-FST)`), pch=1, alpha=1) + 
...
ggsave(plot=gg, filename="facet-fst~km2.png", device=png, width=6, height=3, units="in", dpi=300)
ggsave(plot=gg, filename="facet-fst~km2.pdf", device=cairo_pdf, width=6, height=3, units="in")
