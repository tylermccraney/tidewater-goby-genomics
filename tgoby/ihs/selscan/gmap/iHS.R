...
dot <- 
  left_join(
    x=dot, 
    y=filter(dot, RANK_ABS_iHS>0.99)|>summarise(UPPER_ABS_iHS_99=min(ABS_iHS)), 
    by=join_by(POP)) |>
  left_join(
    y=filter(dot, RANK_ABS_iHS>0.999)|>summarise(UPPER_ABS_iHS_999=min(ABS_iHS)), 
    by=join_by(POP)) |>
  ungroup() |>
...
saveRDS(outliers, file="outliers.p01.RDS")

outliers999 <- filter(dot, RANK_ABS_iHS>0.999)

write_tsv(outliers999, file="outliers.p001.tsv")

brewpal2 <- read_tsv(file="../../../brewpal2.tab")
brewpal2x <- read_tsv(file="../../../brewpal2x.tab")
barplot(1:9, col=brewpal2x$COLORX)

write_tsv(dot, file="iHS.tsv")
saveRDS(dot, file="iHS.RDS")
...
# top 1% of |iHS| considered outliers
gg <- 
  ...
  geom_hline(aes(yintercept=UPPER_ABS_iHS_99), color=brewpal2x$COLORX[4], linewidth=0.5, linetype="dashed") +
  scale_x_continuous(
...
ggsave(plot=gg, filename="iHS.png", device=png, width=8, height=4, units="in", dpi=300)
ggsave(plot=gg, filename="iHS.pdf", device=cairo_pdf, width=8, height=4, units="in")
...
#    stroke_alpha=0.67,
    auto_scale=FALSE) + 
  labs(title="9,735 Candidate Sites (iHS)", tag="c)") + 
...
ggsave(plot=gg.sets_iHS, filename="iHS.SNPs.venn.png", bg="white", device=png, width=5, height=5, units="in", dpi=300)
ggsave(plot=gg.sets_iHS, filename="iHS.SNPs.venn.pdf", bg="white", device=cairo_pdf, width=5, height=5, units="in")
